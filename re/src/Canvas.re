...
let positionAbsolutely: canvasElement => unit = [%bs.raw {|function (node) {
...
module Ctx = {
  ...
  [@bs.send]
  external scale : (ctx, float, float) => unit = "";
 ...
 */
  ...
  [@bs.send] external bezierCurveTo : (ctx, ~c1x: float, ~c1y: float, ~c2x: float, ~c2y: float, ~x: float, ~y: float) => unit = "";
